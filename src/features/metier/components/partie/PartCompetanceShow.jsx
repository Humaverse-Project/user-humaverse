...
const PartCompetanceShow = ({ groupedData, type, titre }) => {
    ...
    return (
        <><Typography variant='h5' sx={{ mt:2,pl:2 }}>{titre}</Typography><hr></hr>
            {Object.keys(groupedData[type]).map((accessitem) => (
                <><></><Grid sx={{ flexGrow: 1, mt: 1, pl: 2 }} container spacing={2}>
                    ...
                    </Grid><Grid item xs={8} key={accessitem + "111"}>
                        {groupedData[type][accessitem].map((datass) => (
                            <><Grid sx={{ flexGrow: 1 }} container spacing={2}><Grid item xs={6} key={accessitem + "44"}>
                                <ListItem sx={{ p: 0 }}> {datass.briquescompetances.brqCompTitre}</ListItem>
                            </Grid>
